...
def validate_sceneflow(model, iters=32, mixed_prec=False):
    ...
    # print(f"Runtime: {format(1 / (end - start), '.2f')} s\n")
    epe_list = np.array(epe_list)
...
def validate_middlebury(model, iters=32, split='F', mixed_prec=False):
    ...
    model.eval()
    aug_params = {}
    val_dataset = datasets.Middlebury1(aug_params, split=split)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--restore_ckpt', help="restore checkpoint", default='./checkpoints/mocha-stereo.pth')
